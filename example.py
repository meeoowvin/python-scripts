            ...
            if parent_id != os.path.basename(base_directory):
                for file_name in files:
                    if not file_name.startswith('.'):  # Skip hidden files
                        ...
                        # Write data to CSV
                        csv_writer.writerow(["Requests", parent_id, attachment_id, file_name, file_size, "Attachment", ""])
                        attachment_id += 1
            ...
            if parent_id != os.path.basename(base_directory):
                for file_name in files:
                    if not file_name.startswith('.'):  # Skip hidden files
                        ...
                        csv_writer.writerow(["Requests", parent_id, attachment_id, new_file_name, file_size, "Attachment", ""])
                        attachment_id += 1
