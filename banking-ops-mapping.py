...
def format_datetime_columns(row):
    columns_to_format = ['Closed time', 'Created time', 'Due by Time', 'Initial response time', 'Resolved time', 'Last update time']
...
input_directory = 'BankingOps-output'
output_directory = 'BankingOps-output-final'
...
for filename in os.listdir(input_directory):
    if filename.endswith('.csv'):
        ...
        df.rename(columns={'Ticket Id': 'Request ID', 'Full name': 'Requester Name', 'Created Time': 'Created Date', 'Type': 'Template Name', 'Subcategory': 'Sub Category', 'Source': 'Mode', 'Agent': 'Technician', 'Closed time': 'Completed Date', 'Resolved time': 'Resolved Time', 'Due by Time': 'Due by date', 'First response status': 'SLA violated', 'Initial response time': 'Responded Date', 'Type of Request': 'Request Type', 'Contact ID': 'Email id', 'Site': 'Program Site'}, inplace=True)
